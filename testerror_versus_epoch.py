...
# --- Training functions ---
# proposed abs
def train_eval_mlp(x_train, x_val, x_test, y_train, y_val, y_true, input_dim, hidden_dim, output_dim, epochs=100):
   # set_random_seed(0)
...
def train_eval_mlp2(x_train, x_val, x_test, y_train, y_val, y_true, input_dim, hidden_dim, output_dim, epochs=100):
    #set_random_seed(0)
...
#SCNN2
def train_eval_cnn(x_train, x_val, x_test, y_train, y_val, y_true, output_dim, epochs=100):
    #set_random_seed(0)
...
#CNN legacy
def train_eval_cnn2(x_train, x_val, x_test, y_train, y_val, y_true, num_classes, epochs=100):
    #set_random_seed(0)
...
errs = {}
titles = ['Proposed method (with original signal)', 'Proposed method (with FFT-processed signal)',
           'Deep learning (MLP with absolute values of signal)', 'Deep learning (CNN with real and imaginary values of signal)',
          'SCNN2']
# hankel
...
errs['Proposed method (with original signal)']=train_eval_mlp(X_train_hk, X_val_hk, X_test_hk, Y_train_oh, Y_val_oh, Y_test, 64, 24, Qmax)
...
errs['Proposed method (with FFT-processed signal)']=train_eval_mlp2(X_train_hkf, X_val_hkf, X_test_hkf, Y_train_oh, Y_val_oh, Y_test, 64, 24, Qmax)

# abs
X_train_abs = np.abs(X_train)
X_val_abs = np.abs(X_val)
X_test_abs = np.abs(X_test)
errs['Deep learning (MLP with absolute values of signal)']=train_eval_mlp(X_train_abs, X_val_abs, X_test_abs, Y_train_oh, Y_val_oh, Y_test, 128, 24, Qmax)
...
errs['Deep learning (CNN with real and imaginary values of signal)'] = train_eval_cnn2(X_train_realimag, X_val_realimag, X_test_realimag, Y_train, Y_val, Y_test, Qmax)
...
errs['SCNN2']=train_eval_cnn(X_train_spec, X_val_spec, X_test_spec, Y_train, Y_val, Y_test, Qmax)
...
plt.figure()
for name, curve in errs.items():
    plt.plot(np.arange(1, len(curve)+1), np.array(curve)*100, label=name)
plt.xlabel('Epoch')
plt.ylabel('Test error (%)')
#plt.title('Test Error vs. Epoch for Each Method')
